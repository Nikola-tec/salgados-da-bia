...
const INITIAL_SHOP_SETTINGS = {
    ...
    whatsappMessage: "Olá! Gostaria de fazer um pedido.", pickupAddress: "Av. Emídio Navarro, Viseu, Portugal",
    pickupCep: "3500-038", storeLatitude: 40.6589, storeLongitude: -7.9138, deliveryPricePerKm: 1.0,
    deliveryMaxRadiusKm: 17, storeTimezone: "Europe/Lisbon", currency: "EUR",
    workingHours: {
        monday: { open: true, start: '10:00', end: '22:00' }, tuesday: { open: true, start: '10:00', end: '22:00' },
        wednesday: { open: true, start: '10:00', end: '22:00' }, thursday: { open: true, start: '10:00', end: '22:00' },
        friday: { open: true, start: '10:00', end: '23:00' }, saturday: { open: true, start: '10:00', end: '23:00' },
...
const useCartTotals = (cartTotal, userData, deliveryFee) => {
    return useMemo(() => {
        ...
        return { subtotal, hasDiscount, discountAmount, finalTotal: Math.max(0, finalTotal) };
...
function App() {
    ...
    const getWorkingInterval = useCallback((workingHours, dateStr) => {
        try {
            ...
            if (hours && hours.open) return { start: hours.start, end: hours.end };
            return null;
        } catch(e) { return null; }
    }, []);
    ...
    const showStoreClosedToast = useMemo(() => { return cart.length > 0 && !storeOpen && view === 'menu'; }, [cart.length, storeOpen, view]);

    const stopTracking = useCallback(() => {
        if (trackerIntervalId) { clearInterval(trackerIntervalId); setTrackerIntervalId(null); setTrackingOrderId(null); }
    }, [trackerIntervalId]); 
    
    const startTracking = useCallback((orderId, initialLat, initialLng) => {
        stopTracking(); setTrackingOrderId(orderId);
        const intervalId = setInterval(async () => {
            const targetOrder = orders.find(o => o.id === orderId);
            if (!targetOrder || !targetOrder.lat || !targetOrder.lng) { stopTracking(); return; }
            const latDiff = targetOrder.lat - currentLat; const lngDiff = targetOrder.lng - currentLng;
            ...
            try { await updateDoc(orderRef, { 'deliveryTracker.lat': newLat, 'deliveryTracker.lng': newLng, 'deliveryTracker.lastUpdate': new Date(), 'deliveryTracker.active': true }); } catch (error) {}
        }, 5000); 
    ...
    const updateUserRole = useCallback(async (uid, newRole) => {
        if (userRole !== 'admin') { showToast("Acesso negado."); return; }
        if (!['customer', 'admin', 'kitchen', 'delivery'].includes(newRole)) return;
        try { await updateDoc(doc(db, `artifacts/${appId}/public/data/users`, uid), { role: newRole }); showToast(`Perfil atualizado para: ${newRole}`); } catch (error) { showToast("Falha ao atualizar."); }
    }, [userRole, showToast]);
    ...
    useEffect(() => {
        ...
        const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
            if (currentUser) {
                ...
                setUserRole(userRoleStatus); setIsAdmin(userRoleStatus === 'admin'); 
                if (userRoleStatus === 'admin') setView('admin'); else if (userRoleStatus === 'kitchen') setView('kitchenView'); else if (userRoleStatus === 'delivery') setView('deliveryView'); else setView('menu');
                setIsAuthReady(true);
            } else {
                signInAnonymously(auth).catch(err => { setError("Não foi possível carregar o cardápio."); }).finally(() => {
                    setUserData({ name: 'Anônimo', email: '', addresses: [], hasGivenFeedback: false, hasFeedbackDiscount: false, role: 'customer' });
                    setUserRole('customer'); setIsAuthReady(true); setView('menu');
                });
            }
        });
    ...
    useEffect(() => { if (userRole !== 'delivery') return; return () => stopTracking(); }, [stopTracking, userRole]);
    ...
    const addToCart = (item, customization, priceOverride) => {
        ...
        setCart(prevCart => {
            const existingItem = prevCart.find(ci => ci.id === item.id && JSON.stringify(ci.customization || null) === JSON.stringify(customization || null));
            if (existingItem) {
                return prevCart.map(ci => ci.id === item.id && JSON.stringify(ci.customization || null) === JSON.stringify(customization || null) ? { ...ci, quantity: ci.quantity + 1 } : ci);
            }
            const newItem = { ...finalItem, quantity: quantityToAdd };
    ...
    const handleLogin = async (email, password) => {
        ...
        try {
            ...
            if(userRoleStatus === 'admin') setView('admin'); else if (userRoleStatus === 'kitchen') setView('kitchenView'); else if (userRoleStatus === 'delivery') setView('deliveryView'); else setView('cart');
    ...
    const updateOrderStatus = async (orderId, status) => {
        ...
        if (status === 'Concluído' && trackingOrderId === orderId) { stopTracking(); updateData['deliveryTracker.active'] = false; }
        if (status === 'Saiu para Entrega') {
            if (orderToUpdate.deliveryMethod === 'deliver' && trackingOrderId !== orderId) {
                 ...
                 updateData['deliveryTracker.active'] = true; updateData['deliveryTracker.lastUpdate'] = new Date();
                 updateData['deliveryTracker.lat'] = orderToUpdate.lat || currentLat; updateData['deliveryTracker.lng'] = orderToUpdate.lng || currentLng; 
            }
        } else if (orderToUpdate?.deliveryTracker?.active) { updateData['deliveryTracker.active'] = false; }

        if (status === 'Rejeitado') { await deleteDoc(orderRef); showToast(`Pedido #${orderId.slice(0, 8)} rejeitado e removido.`); return; }
...
const FirebaseErrorScreen = () => (
    <div className="flex flex-col items-center justify-center h-screen bg-red-50 text-red-800 p-4 text-center">
        ...
        <p className="mt-2 max-w-md">Não foi possível estabelecer ligação à base de dados.</p>
    </div>
);
...
const ConfirmDeleteModal = ({ onConfirm, onCancel, title="Confirmar Exclusão", message="Tem a certeza que quer apagar este item?", confirmText="Apagar" }) => (
...
const MenuView = ({ menu, addToCart, showStoreClosedToast }) => {
    ...
    // Adicionei "Box" no singular aqui por segurança
    const ordemDesejada = [
    ...
    // A MÁGICA ACONTECE AQUI: Agora ele puxa tanto "Box" quanto "Boxes"
    const boxesMenu = menu.filter(item => (item.category === 'Boxes' || item.category === 'Box') && item.isAvailable !== false);
    
    // E aqui garantimos que "Box" ou "Boxes" não apareçam lá embaixo no cardápio normal
    const otherCategories = categories.filter(category => category !== 'Boxes' && category !== 'Box');
        ...
        <div className="animate-fade-in relative">
            ...
            {boxesMenu.length > 0 && (
                <div className="relative mb-12 -mx-4 md:-mx-6 px-4 md:px-6 py-8 bg-amber-50 border-y border-amber-200 overflow-hidden shadow-inner">
                    ...
                    {/* Fundo Estilizado com Coxinhas e Corações */}
...
const FeedbackForm = ({ onSubmit }) => {
    const [rating, setRating] = useState(0); const [howFound, setHowFound] = useState(''); const [wouldRecommend, setWouldRecommend] = useState(''); const [isSubmitting, setIsSubmitting] = useState(false);
