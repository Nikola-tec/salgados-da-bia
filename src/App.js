...
function App() {
    ...
    const renderView = () => {
        if (userRole === 'admin') {
             switch (view) {
                ...
                case 'crm': 
                    return <CRMView orders={orders} setView={setView} db={db} showToast={showToast} />;
            }
        }
    ...
    return (
        <div className="bg-stone-50 min-h-screen font-sans text-stone-800" style={{fontFamily: "'Inter', sans-serif"}}>
            <NotificationSetup user={user} userRole={userRole} appId={appId} />
...
const CRMView = ({ orders, setView, db, showToast }) => {
    ...
    const appId = "salgados-da-bia"; // Garanta que o ID seja consistente

    // Carrega usuários e enriquece com dados de compras
    useEffect(() => {
            ...
            try {
                ...
                const enrichedUsers = usersData.map(user => {
                    ...
                    const lastOrderDate = userOrders.length > 0 
                        ? new Date(Math.max(...userOrders.map(o => o.createdAt?.seconds * 1000 || 0))) 
                        : null;
                    const totalSpent = userOrders.reduce((acc, curr) => acc + (curr.total || 0), 0);
                    
                    return { ...user, orderCount: userOrders.length, lastOrderDate, totalSpent };
                });

                // Ordena: quem comprou mais recentemente aparece primeiro
                enrichedUsers.sort((a, b) => (b.lastOrderDate || 0) - (a.lastOrderDate || 0));
                setUsers(enrichedUsers);
                setLoading(false);
            } catch (error) {
                console.error("Erro ao carregar usuários:", error);
                showToast("Erro ao carregar lista de clientes.");
                setLoading(false);
            }
        };
        fetchUsers();
    ...
    const handleSendCampaign = async () => {
        if (!campaignTitle.trim() || !campaignBody.trim()) return showToast("Preencha título e mensagem.");
        if (selectedUsers.length === 0) return showToast("Selecione clientes.");

        setSending(true);
        try {
            // Salva a campanha no banco -> Isso dispara a Cloud Function que criamos!
            await addDoc(collection(db, `artifacts/${appId}/public/data/marketing_campaigns`), {
                title: campaignTitle,
                body: campaignBody,
                targetUids: selectedUsers, // Envia para quem foi selecionado
                createdAt: new Date(),
                status: 'Pendente',
    ...
    if (loading) return <div className="p-8 text-center"><Loader2 className="animate-spin inline text-amber-600"/> Carregando CRM...</div>;

    return (
        <div className="space-y-6 pb-20">
            <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold text-stone-800 flex items-center"><Megaphone className='mr-3 text-amber-600'/> Marketing & CRM</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="font-bold text-lg flex items-center gap-2"><Users size={20}/> Clientes ({users.length})</h3>
                        <button onClick={selectAll} className="text-sm text-amber-600 font-bold hover:underline">
                    ...
                    <div className="overflow-y-auto max-h-[500px] pr-2 space-y-2">
                        {users.map(user => (
                            <div key={user.id} onClick={() => toggleSelectUser(user.id)} className={`flex justify-between items-center p-3 rounded-lg border cursor-pointer transition-all ${selectedUsers.includes(user.id) ? 'border-amber-500 bg-amber-50' : 'border-stone-100 hover:bg-stone-50'}`}>
                                <div className="flex items-center gap-3">
                                    <div className={`w-5 h-5 rounded border flex items-center justify-center ${selectedUsers.includes(user.id) ? 'bg-amber-500 border-amber-500' : 'border-stone-300'}`}>
                                ...
                                <div className="text-right text-xs">
                                    <p className="text-stone-600 font-semibold">{user.orderCount} Pedidos</p>
                                    <p className="text-stone-400">Último: {user.lastOrderDate ? user.lastOrderDate.toLocaleDateString() : 'Nunca'}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm h-fit sticky top-4">
                    <h3 className="font-bold text-lg mb-4 flex items-center gap-2"><Send size={20}/> Nova Notificação Push</h3>
                    <div className="space-y-4">
                        <input type="text" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-amber-500 outline-none" placeholder="Título (ex: Promoção Relâmpago!)" value={campaignTitle} onChange={e => setCampaignTitle(e.target.value)} />
                        <textarea className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-amber-500 outline-none h-32 resize-none" placeholder="Mensagem para o cliente..." value={campaignBody} onChange={e => setCampaignBody(e.target.value)} />
                        <button onClick={handleSendCampaign} disabled={sending || selectedUsers.length === 0} className="w-full bg-green-600 text-white font-bold py-3 rounded-xl hover:bg-green-700 transition-colors shadow-lg flex justify-center items-center gap-2 disabled:opacity-50">
                            {sending ? <Loader2 className="animate-spin"/> : <Send size={20}/>} {sending ? 'Enviando...' : 'Enviar Agora'}
                        </button>
                    </div>
                </div>
            </div>
