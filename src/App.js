    ...
    return (
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg animate-fade-in">
            ...
            <div className="flex flex-wrap gap-2 mb-6 border-b">
                ...
                <button onClick={() => setAdminView('crm')} className={`px-4 py-2 font-semibold text-sm rounded-t-xl flex items-center gap-2 transition-colors ${adminView === 'crm' ? 'bg-stone-100 border-b-2 border-amber-500 text-amber-600' : 'text-stone-500 hover:bg-stone-100'}`}><Megaphone size={16}/> CRM</button>
            </div>
            {renderAdminView()}
        </div>
    );
